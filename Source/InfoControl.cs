...
namespace Lens
{
   ...
   internal class InfoControl
   {
      ...
      public void UpdateInfo(Point mousePosition, Color color)
      {
         ...
         this.ValueColor12Bit = ColorAs12Bit(color);
         this.ValueColorWeb   = ColorAsWeb(color);
      ...
      private static string ColorAs12Bit(Color color)
      {
         ...
         return ColorAsHex(Color.FromArgb(Snap(color.R), Snap(color.G), Snap(color.B)), shortForm: true);
      }

      private static string ColorAsWeb(Color color)
      {
