...
- **Distraction-free mode** — Ctrl+Alt+Shift+D, focus-scoped to `LensForm` (local `OnKeyUp` handler, not a global hotkey like the tray's Z/X/R/S/W/D1/Q). Toggles `Lens.DistractionFreeActive`, an in-memory-only flag on the `Lens` singleton (survives Lens window close/reopen within the process, resets on relaunch — never persisted to settings.json). While active, grid/crosshair/info-panel visibility and the scaling mode are driven by `Lens.DistractionFree` (`DistractionFreeSettings`) instead of the normal user settings — a config-file-only block with no Settings UI, defaulting to grid off, crosshair off, info panel off (`ShowInfo` hides the whole panel, not just the color rows), `HighQualityBicubic` scaling. The crosshair and the sampled-pixel box are not independently toggleable: `LensForm.ApplyDifferenceCrosshair` draws the box's top/left edges as literally the same bands as the crosshair lines, so hiding one hides the other.
